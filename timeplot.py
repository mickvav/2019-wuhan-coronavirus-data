...
X, t01 = plot_linear({t:v for t,v in values_diff.items() if t-t0>10*3600*24},3,"diff(log(total_deaths)) last")
#
...
X, t02 = plot_linear({t:v for t,v in values_diff.items() if t-t0>10*3600*24 and tmax-t > 2*24*3600},3,"diff(log(total_deaths)) last but 2 days")
...
X, t02 = plot_linear({t:v for t,v in values_diff.items() if t-t0>10*3600*24 and tmax-t > 4*24*3600},3,"diff(log(total_deaths)) last but 4 days")
