...
def count_neighbours(ts, interval):
    """
    For each element of sorted array of timestamps, count number of 
    other timestamps, that belong to [ts-interval, ts+interval]
    ...
    for i in ts:
        n = 0
        for j in ts:
            if j >= i - interval and j <= i + interval:
                n += 1
        result.append(n)
    return result
...
def plot_linear(vls, column=3, label="fit"):
    ...
    nb = count_neighbours(ts, 12 * 3600)
    W = np.sqrt(np.diag([(i + 1) ** 2 / nb[i] for i in range(len(vls))]))
    X, residuals, rank, s = np.linalg.lstsq(np.dot(W, A), np.dot(B, W))

    if "--show" in argv:
        _ = plt.plot(
            [(t - t0) / dt for t in ts],
            [vls[t][column] for t in ts],
            "o",
            label="Data",
            markersize=3,
        )
...
t0 = ts[0]
A = np.vstack(
    [[((t - t0) / dt) ** 2 for t in ts], [(t - t0) / dt for t in ts], np.ones(len(ts))]
).T
...
if "--show" in argv:
    ...
    _ = plt.plot(
        [(t - t0) / dt for t in ts],
        [X[0] * ((t - t0) / dt) ** 2 + X[1] * ((t - t0) / dt) + X[2] for t in ts],
        "r",
        label="fit",
    )
    _ = plt.legend()
...
if "--show" in argv:
    _ = plt.plot(
        range(100),
        [X[0] * t ** 2 + X[1] * t + X[2] for t in range(100)],
        "r",
        label="prognosis",
    )
    plt.show()
...
#
plot_linear(
    {t: v for t, v in values_diff.items() if t - t0 > 10 * 3600 * 24},
    2,
    "diff(log(total)) last",
)
X, t01, t_max1 = plot_linear(
    {t: v for t, v in values_diff.items() if t - t0 > 10 * 3600 * 24},
    3,
    "diff(log(total_deaths)) last",
)
#
...
if "--show" in argv:
    _ = plt.plot(
        range(100),
        [X[0] * t + X[1] for t in range(100)],
        "r",
        label="prognosis(diff(log(total)deaths))",
    )
    plt.show()
#
...
X, t02, t_max2 = plot_linear(
    {
        t: v
        for t, v in values_diff.items()
        if t - t0 > 10 * 3600 * 24 and tmax - t > 2 * 24 * 3600
    },
    3,
    "diff(log(total_deaths)) last but 2 days",
)
...
X, t02, t_max2 = plot_linear(
    {
        t: v
        for t, v in values_diff.items()
        if t - t0 > 10 * 3600 * 24 and tmax - t > 4 * 24 * 3600
    },
    3,
    "diff(log(total_deaths)) last but 4 days",
)
...
if "--show" in argv:
    _ = plt.plot(
        ...
        [X[0] * t + X[1] for t in range(100)],
        "r",
        label="prognosis(diff(log(total)))",
    )
    plt.show()
#
